...
class Hotel extends CI_Controller {
	...
	public function index()
	{
		...
		$data['roomdetails'] =$this->db->select("roomdetails.*,room_image.room_imagename")->from('roomdetails')->join('room_image','room_image.room_id=roomdetails.roomid','left')->order_by('rate','ASC')->limit('4')->get()->result();
	...
	public function roomdetails()
	{
		...
		$roomdetails=$this->db->select("roomdetails.*,room_image.room_imagename,room_image.room_gallery")->from('roomdetails')->join('room_image','room_image.room_id=roomdetails.roomid','left')->where('roomid',$roomid)->get()->row();
...
function _alpha_dash_space($str_in = '',$fields=''){
 ...
 public function bookedroom(){
    ...
    public function room($prams){
        $id = $prams;
		$data['title'] = "Room details";
		$roomid = $id;
		$data['roomdetails'] = $this->db->select("roomdetails.*,room_image.room_imagename,room_image.room_gallery")->from('roomdetails')->join('room_image','room_image.room_id=roomdetails.roomid','left')->where('roomid',$roomid)->get()->row();
		$data['amenities'] = $this->db->select("*")->from('room_amenities')->where('room_id',$roomid)->get()->result();
...
// 		}
    ...
    public function paymentgateway($orderid,$paymentid){
    		  ...
    		  if($paymentid==1){
    			...
    			$this->load->library('DPOPayment');
    
                echo "<h3>Wait...DirectPay(DPO) Payment Processing....</h3>";
        			
                $payment = new DPOPayment(['companyToken'=>$data['paymentinfo']->marchantid,
                "serviceType"=>$data['paymentinfo']->service_type,'endpoint'=>$data['paymentinfo']->endpoint]);
            
                // Payment data
                $postdata = array(
                    'paymentAmount' => $amount,
                    'PaymentCurrency' => $data['paymentinfo']->currency, 
                    ...
                    'customerPhone' => $phone,
                    'reference' => $orderid,
                    'redirectURL' => base_url().'hotel/successful/'.$orderid.'/'.$paymentid,
                    'backUrl' => base_url()."hotel/cancilorder/".$orderid
                ); 
            
                // Initiate payment and get the payment URL
                $res = $payment->createToken($postdata);
                if($res['success']){
                    header('Location:'.$data['paymentinfo']->payment_url.$res['transToken']);
                    return;
                }else{
                    $this->session->set_userdata('exception', display($res['error']));
	...
	public function successful($orderid = '',$paymentid =''){
        ...
        if(!empty($data)){
            file_put_contents(getcwd().'/logs/callback.log',json_encode($data), FILE_APPEND);
            // $order = $_GET("orderid");
	...
	public function fail($orderid){
	    ...
	    if(!empty($_SESSION['message'])){
    		...
    		$this->session->set_userdata('exception', display($message));
    	 redirect('hotel/orderdelevered/');		
		}	
	...
	public function orderdelevered($orderid=NULL){
		if(empty($orderid)){
			redirect('');
		}
