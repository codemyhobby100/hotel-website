	  ...
	  $img = $this->fileupload->do_upload(
        ...
        if(count($files) > 0){
            foreach ($files['name'] as $key => $name) {
                ...
                $fileName = md5(basename($name)).".".$fileExtension;
    
                $destination = getcwd().'/application/modules/room_setting/assets/images/'.date("Y-m-d");
                $file = 'application/modules/room_setting/assets/images/'.$fileName;
	  ...
	  if ($this->form_validation->run()) { 
	   if(empty($this->input->post('room_img_id', TRUE))) {
    	   ...
    	   $data['room_setting']   = (Object) $postData = array(
            		   ...
            		   'room_imagename' 	         => $img,
            		   'room_gallery'               => $gallerya
            		  );
            		  
    		$this->permission->method('room_setting','create')->redirect();
    		$sql = $this->roomimage_model->create($postData);
